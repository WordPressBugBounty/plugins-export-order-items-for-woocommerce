...
function hm_xoiwc_on_before_woocommerce_init()
{
...
add_action('before_woocommerce_init', 'hm_xoiwc_on_before_woocommerce_init');

function hm_xoiwc_is_hpos() {
...
function hm_xoiwc_on_init() {
    ...
    if ($pagenow == 'admin.php' && isset($_GET['page']) && $_GET['page'] == 'hm_xoiwc') {
		...
		if (current_user_can('view_woocommerce_reports') && !empty($_POST['hm_xoiwc_do_export'])) {
			...
			update_option('hm_xoiwc_report_settings', $savedReportSettings, false);
...
function hm_xoiwc_get_report_dates() {
...
function hm_xoiwc_export_body($dest, $return = false) {
	...
	list($start_date, $end_date) = hm_xoiwc_get_report_dates();
        ...
        foreach ($_POST['fields'] as $field) {
            switch ($field) {
                ...
                case 'order_status':
                    ...
                    break;
                case 'order_date':
                    $row[] = $isHpos ? get_date_from_gmt($product->order_date) : $product->order_date;
                    break;
                case 'product_sku':
...
function hm_xoiwc_run_scheduled_report($reportId, $start, $end, $args = array(), $output = false) {
    $savedReportSettings = get_option('hm_xoiwc_report_settings', [ hm_xoiwc_default_report_settings() ]);
