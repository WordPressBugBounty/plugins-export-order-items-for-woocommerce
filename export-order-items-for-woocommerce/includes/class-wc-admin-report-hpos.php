...
class WC_Admin_Report_HPOS_WPZ_XOIF extends WC_Admin_Report {
	...
	public static function getGmtConversionSql($field) {
		...
		for ($i = count(self::$tzTransitions) - 1; $i > 0; --$i) {
			$sql .= 'IF('.$field.'>="'.date('Y-m-d H:i:s', self::$tzTransitions[$i]['ts']).'",DATE_ADD('.$field.', INTERVAL '.((int) self::$tzTransitions[$i]['offset']).' SECOND),';
		}
	...
	public function get_order_report_data( $args = array() ) {
		...
		foreach ( $data as $raw_key => $value ) {
			...
			if ($raw_key == 'date_created_gmt_wpz') {
				$get_key = self::getGmtConversionSql($get_key);
			}
